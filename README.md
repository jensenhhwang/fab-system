...
- Planning Bridge: 수동 입고계획의 초안·확정·취소와 WMS 부분입고·완료 연결
...
공정별 사용량 → 일사용량·DOH → 재고/ROP
                                ↓
수동 Planning Bridge → WMS 로트·위치 → MES 작업지시·BOM → FEFO 피킹 → 재고 이동
```
...
필요하면 `db:migrate-storage`, `db:migrate-warehouse-ops`, `db:migrate-procurement`, `db:migrate-inbound-plans`, `db:validate` 스크립트로 운영 구조를 준비하고 검증합니다.
...
- 외부 ERP와 직접 연결하지 않습니다. Planning Bridge는 사용자가 입력한 입고계획을 WMS 실행과 연결하는 제한형 계획 모듈이며 회계·세무·결산·외부 거래를 처리하지 않습니다.
- 생산계획에서 MES 작업지시로 이어지는 계획 연결은 아직 구현되지 않았습니다.
