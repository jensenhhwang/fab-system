...
import {
  ...
  isReservedWarehouseHandlingUnit,
  isWarehousePhysicalHandlingUnit,
...
async function main() {
  ...
  const rows = materialDocs.map(material => {
    ...
    const lots = lotDocs.filter(item => item.materialId === material._id && item.qualityStatus !== "CONSUMED");
    const units = unitDocs.filter(item => item.materialId === material._id && isWarehousePhysicalHandlingUnit(item));
    ...
    for (const unit of units.filter(isReservedWarehouseHandlingUnit)) {
    ...
    const lotPhysical = lots.reduce((sum, item) => sum + item.availableQuantity + (reservedByLot.get(item._id) ?? 0), 0);
    const huPhysical = units.reduce((sum, item) => sum + item.quantity, 0);
    ...
    return {
      ...
      lotCount: lots.length, lotAvailable, lotRestricted, lotPhysical,
      huCount: units.length, huPhysical,
  ...
  const mismatchedLot = rows.filter(row => Math.abs(row.inventoryAvailable - row.lotPhysical) > RECONCILIATION_TOLERANCE);
  const mismatchedHu = rows.filter(row => Math.abs(row.inventoryAvailable - row.huPhysical) > RECONCILIATION_TOLERANCE);
  const activeLotIds = new Set(lotDocs.filter(lot => lot.qualityStatus !== "CONSUMED").map(lot => lot._id));
  const orphanHu = unitDocs.filter(unit => isWarehousePhysicalHandlingUnit(unit) && !activeLotIds.has(unit.inventoryLotId));
  ...
  console.log(`[inventory-audit] criticalUnder5d=${critical.length} near2d=${nearTwo.length} usageScaleMismatch=${scaleMismatch.length} lotCovered=${lotCovered.length} lotMismatch=${mismatchedLot.length} huMismatch=${mismatchedHu.length} orphanHu=${orphanHu.length}`);
  ...
  for (const row of mismatchedLot) console.log(`${row.code}\tinventory=${row.inventoryAvailable}\tlotPhysical=${row.lotPhysical}\trestricted=${row.lotRestricted}\tdelta=${(row.lotPhysical - row.inventoryAvailable).toFixed(2)}`);
  console.log("\n[Handling Unit projection 불일치]");
  for (const row of mismatchedHu.slice(0, 30)) console.log(`${row.code}\tinventory=${row.inventoryAvailable}\thuPhysical=${row.huPhysical}\tdelta=${(row.huPhysical - row.inventoryAvailable).toFixed(2)}`);
