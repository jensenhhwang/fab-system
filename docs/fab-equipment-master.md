...
| M21 | DDR5 16Gb | 184,000 WSPM | 병목 ≥15% · 비병목 ≥25% | **495대** | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |
| M22 | NAND 321단 1Tb TLC | 108,000 WSPM | 병목 ≥15% · 비병목 ≥25% | **533대** | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |
...
| 합계 |  |  | **495** |  | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |
...
**총 대수(495대)가 M20(329대)보다 많은 이유**: M21의 WSPM(184,000)은 M20(117,000)보다 57% 크다. P08(TSV) 공정군이 통째로 없고 P09도 절반(pass 1 vs 2)이라 wafer당 필요 설비는 그만큼 적지만, 그 감소분보다 WSPM 증가분이 더 커서 총 대수는 M20보다 많다.
...
| 합계 |  |  | **533** |  | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |

P01~P07·P09는 M20과 동일한 `ratedWph`를 재사용하되(동일 세대 장비 기술이라는 가정), pass count는 M22의 route-master.md 표(§ M22 · NAND)에서 그대로 가져왔다. **M22 총 대수(533)가 3팹 중 가장 많다** — M22의 NORMAL WSPM(108,000)은 M20(117,000)보다 낮지만 P02 대수는 M20 병목 공정(P04 56대)의 5배가 넘는 319대다. 이는 321단 적층으로 인한 CVD 반복 방문수(162 pass/wafer)가 다른 어떤 공정보다도 압도적으로 크기 때문이며, NAND Fab이 DRAM/HBM Fab보다 wafer당 장비 시간을 훨씬 많이 소비한다는 업계 통설과 일치한다([Lam Research](https://newsroom.lamresearch.com/learning-from-nand-3d-dram-transition-ai-era)).
...
- 따라서 329대(M20)·495대(M21)·533대(M22)는 모두 `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE`이며 실제 MES 정격·qualification을 반영한 `REAL_CAPACITY_VALIDATED`가 아니다.
...
- `src/lib/equipment-wph-model.ts`: `modeledOeeForSequence`/`supportedWspm`/`minimumDefinedCount`/`resolveTargetLoad` — 3팹이 공유하는 공식
