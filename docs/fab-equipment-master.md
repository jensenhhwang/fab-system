...
| M21 | DDR5 16Gb | 73,600 WSPM | ≥15% | **198대** | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |
| M22 | NAND 321단 1Tb TLC | 90,000 WSPM | ≥15% | **427대** | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |
...
M21은 P08(TSV) 전 공정이 없고, P09는 1회(EDS)만 필요하다([`route-master.md`](./route-master.md#m21--dram)). NORMAL 73,600 WSPM과 M21의 route pass count로 M20과 동일한 WPH_PROXY 공식을 새로 적용했다.

| 공정 | 대표 공정 | Route pass/wafer | 표준 대수 | 계획 부하 | Capacity 상태 |
|---|---|---:|---:|---:|---|
| P01 | 산화 | 4 | **5** | 83.7% | `WPH_PROXY` |
| P02 | CVD | 27 | **37** | 83.2% | `WPH_PROXY` |
| P03 | 포토 | 27 | **42** | 83.6% | `WPH_PROXY` |
| P04 | 식각 | 27 | **36** | 83.1% | `WPH_PROXY` |
| P05 | 이온주입 | 6(M20과 동일 가정) | **7** | 84.9% | `WPH_PROXY · 6 PASS/WAFER` |
| P06 | 금속배선 | 5 | **7** | 80.9% | `WPH_PROXY` |
| P07 | CMP | 27 | **39** | 84.4% | `WPH_PROXY` |
| P09 | 웨이퍼 테스트 | 1(적층 없어 재검 불필요) | **2** | 47.1% | `WPH_PROXY` |
| P10 | Conventional 단일 다이 패키징 | — | **23** | `RATE_TBD` | `NATIVE_STAGE_PROXY · RATE_TBD` |
| 합계 |  |  | **198** |  | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |

P01~P07·P09는 M20과 동일한 `ratedWph`(P01=115, P02=105, P03=92, P04=108, P05=120, P06=105, P07=98, P09=130 wafer/hour)를 재사용한다 — DRAM 프런트엔드가 HBM과 같은 물리 공정·장비군을 쓴다는 근거([Applied Materials](https://www.appliedmaterials.com/us/en/newsroom/blogs/hbm--materials-innovation-propels-high-bandwidth-memory-into-the.html))에 따른 것이며, 대수 자체는 M21 고유의 WSPM·pass count에서 새로 계산한 결과다. P09는 M20의 절반(pass 1 vs 2)이라 대수도 뚜렷이 적다.

### P10 Conventional Packaging Capacity 계약 — `RATE_TBD`

M21의 P10은 M20처럼 다이 적층(Bonding)이 아니라 리드프레임/기판 실장이라 설비 카테고리 자체가 다르다([SK hynix Newsroom](https://news.skhynix.com/semiconductor-back-end-process-episode-6-conventional-packages/)). Wire bonder·molding press·trim-form/ball-mount 장비의 공개 UPH(시간당 처리량) 데이터를 확보하지 못해, 아래 대수는 M20 P10(36대 / 117,000 WSPM)과 동일한 대수-대-WSPM 비율을 M21 WSPM(73,600)에 적용한 **자릿수 placeholder**다. 실제 vendor spec이 들어오기 전까지 정격(`ratedRate`)은 `RATE_TBD`로 두고 계획 부하율을 계산하지 않는다.

| 단계 | 대수(placeholder) | 상태 |
|---|---:|---|
| Backgrind(단일-패스) | 2 | `RATE_TBD` |
| Dicing / Singulation | 3 | `RATE_TBD` |
| Die Attach | 3 | `RATE_TBD` |
| Wire Bond | 7 | `RATE_TBD` |
| Molding | 3 | `RATE_TBD` |
| Lead Finish / Ball Mount | 2 | `RATE_TBD` |
| Final Test | 3 | `RATE_TBD` |
| 합계 | **23** | `RATE_TBD` |
...
|---|---:|---:|---:|---|
| `NORMAL` | 73,600 | P05 | 약 84.9% | 15% reserve 충족 |
| `UPLIFT` | 76,800 | P05 | 약 88.6% | 단기 증산 |
| `NAMEPLATE` | 80,000 | P05 | 약 92.3% | 스트레스 운전 |
| `EXPANSION` | 88,000 | P05 | 약 101.5% | 별도 증설 필요 |

---

## M22 표준 설비 대수

M22는 321단 3-deck NAND의 route pass count(`route-master.md` 참고)가 반영되어 P02(CVD)가 압도적으로 많다. NORMAL 90,000 WSPM 기준으로 계산했다.

| 공정 | 대표 공정 | Route pass/wafer | 표준 대수 | 계획 부하 | Capacity 상태 |
|---|---|---:|---:|---:|---|
| P01 | 산화(주변 CMOS) | 3 | **5** | 76.7% | `WPH_PROXY` |
| P02 | CVD(스택 증착 161 + ONO 1) | 162 | **266** | 84.8% | `WPH_PROXY` |
| P03 | 포토(주변 3 + Staircase 21) | 24 | **45** | 84.8% | `WPH_PROXY` |
| P04 | 식각(주변 3 + 채널홀 3 + Staircase 21 + 게이트치환 3) | 30 | **48** | 84.6% | `WPH_PROXY` |
| P05 | 이온주입(주변 CMOS) | 3 | **5** | 73.5% | `WPH_PROXY · M20 동일 가정 재사용` |
| P06 | 금속배선(주변 3 + 게이트치환 텅스텐 3) | 6 | **10** | 83.9% | `WPH_PROXY` |
| P07 | CMP(주변 3 + 평탄화 7) | 10 | **18** | 83.1% | `WPH_PROXY` |
| P09 | 웨이퍼 테스트 | 1 | **2** | 57.6% | `WPH_PROXY` |
| P10 | 16단 와이어본딩 패키징 | — | **28** | `RATE_TBD` | `NATIVE_STAGE_PROXY · RATE_TBD` |
| 합계 |  |  | **427** |  | `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE` |

P01~P07·P09는 M20과 동일한 `ratedWph`를 재사용하되(동일 세대 장비 기술이라는 가정), pass count는 M22의 route-master.md 표(§ M22 · NAND)에서 그대로 가져왔다. **M22 총 대수(427)가 M20(494)보다 적지만 NORMAL WSPM은 M20의 약 77%(90,000/117,000)에 불과하다** — 그런데도 P02·P03·P04 개별 대수는 M20보다 훨씬 많다(P02 266 vs 58). 이는 321단 적층으로 인한 증착·식각 반복 방문수 증가(162 vs 27, 30 vs 27) 때문이며, NAND Fab이 DRAM/HBM Fab보다 wafer당 장비 시간을 훨씬 많이 소비한다는 업계 통설과 일치한다([Lam Research](https://newsroom.lamresearch.com/learning-from-nand-3d-dram-transition-ai-era)).

### P10 와이어본딩 패키징 Capacity 계약 — `RATE_TBD`

M21과 동일한 이유로 wire bonder 등의 공개 UPH를 확보하지 못했다. M20 P10 비율(36대/117,000 WSPM)을 M22 WSPM(90,000)에 적용한 placeholder이며, 16단 적층 특성상 Wire Bond 단계 비중을 M21보다 높게 배분했다.

| 단계 | 대수(placeholder) | 상태 |
|---|---:|---|
| Backgrind | 2 | `RATE_TBD` |
| Dicing / Singulation | 3 | `RATE_TBD` |
| Die Attach | 4 | `RATE_TBD` |
| Wire Bond(16단 적층) | 10 | `RATE_TBD` |
| Molding | 3 | `RATE_TBD` |
| Lead Finish / Ball Mount | 2 | `RATE_TBD` |
...
| 시나리오 | WSPM | 병목 공정 | 병목 부하율(P01~P09 기준) | 판정 |
|---|---:|---:|---:|---|
| `NORMAL` | 90,000 | P02 | 약 84.8% | 15% reserve 충족 |
| `UPLIFT` | 95,000 | P02 | 약 89.5% | 단기 증산 |
| `NAMEPLATE` | 100,000 | P02 | 약 94.2% | 스트레스 운전 |
| `EXPANSION` | 110,000 | P02 | 약 103.6% | 별도 증설 필요 |
...
- 따라서 494대(M20)·198대(M21)·427대(M22)는 모두 `INDUSTRY_RANGE_INFORMED_MODELED_BASELINE`이며 실제 MES 정격·qualification을 반영한 `REAL_CAPACITY_VALIDATED`가 아니다.
