...
- **실제 발주 생성·전송** — 하지 않는다. `purchaseOrderDrafts`/`integrationOutbox`(레거시 경로)를 만들지 않는다.
- **실제 입고 등록** — 하지 않는다. `inventoryLots`/`inventoryMovements`에 아무것도 쓰지 않는다.
- **재고 이동·피킹** — WMS 에이전트 영역이다 (미설계, [§8](#8-관련-문서와-구현)).
- **생산계획 자동 수립** — MES/PROCESS 영역이다.
- **What-if 시나리오 자동 반영** — 지금은 이벤트가 없는 "현재 재고 기준 위험 점검"만 돈다. What-if 화면에서 세운 시나리오를 이 에이전트가 자동으로 읽어오지 않는다 ([§7 MVP-1](#7-로드맵)에서 연결 예정).
...
| MVP-2 | L4 자재에 한해 잠정입고(QUARANTINE 로트) 자동 실행 + 원클릭 롤백. 위험물/단일소싱은 계속 L2 고정 | ❌ 미착수 |
