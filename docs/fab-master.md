...
이 문서는 M20이 **평균적으로 얼마나 생산하고, 정상 상태에서 얼마의 WIP를 유지하며, 현재 설비 가정으로 어디까지 증산할 수 있는지**를 정의하는 생산 규모의 기준 문서다.
...
이 세 문서의 값이 충돌하면 임의로 숫자를 맞추지 않는다. 생산 규모는 이 문서, 공정 방문수는 `route-master.md`, 자재 원단위는 `material-consumption-master.md`를 각각 단일 기준으로 사용한다.
...
| 기준 생산품 | `M20-HBM4-12H-V1` | DRAM base die + TSV + 12-Hi 36GB급 적층 참조 모델 |
| 웨이퍼 직경 | 300 mm | WSPM과 FOUP 환산 기준 |
| 명목 생산능력 | 130,000 WSPM | 현 설비 모델의 월 wafer-start 상한 |
...
| 시나리오 | 월 투입 WSPM | 현재 명목 대비 가동률 | NORMAL 대비 | 105일 고정 참조 WIP | 계획 cycle time | 계획 WIP | HBM4 12-Hi 환산/월 | 운영 판정 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| `NORMAL` | 117,000 | 90% | 기준 | 16,380 | 105일 | 16,380 | 5,703,750 | 평균 운영 기준 |
| `UPLIFT` | 123,500 | 95% | +5.6% | 17,290 | 112일 | 18,443 | 6,020,625 | 무투자 단기 증산 상한 |
| `NAMEPLATE` | 130,000 | 100% | +11.1% | 18,200 | 126일 | 21,840 | 6,337,500 | 비상·단기 스트레스, 지속운전 기준 아님 |
| `EXPANSION` | 143,000 | 현 명목의 110% | +22.2% | 20,020 | 105일 | 20,020 | 6,971,250 | 디보틀넥·설비증설 후 미래 상태 |

### 해석 규칙

- 현재 M20 가정이 **운영상 커버하는 지속 가능한 증산 룸은 `UPLIFT`의 123.5K WSPM**, NORMAL 대비 약 5.6%다.
- 130K는 설비 모델상 이름표 상한이다. 일부 기간 도달할 수 있는 스트레스 시나리오지만 100% 가동과 105일 사이클을 동시에 지속할 수 있다고 가정하지 않는다.
...
| 이론 HBM4 12-Hi stack | 650 ÷ 12 dies | 약 54.167 stacks |
| 양품 HBM4 12-Hi | 54.167 × 적층·조립수율 90% | **48.75 stacks** |

따라서 M20 V1은 **wafer 1장당 평균 양품 DRAM die 650개, 최종 양품 HBM4 12-Hi 48.75개**를 생산하는 것으로 가정한다. 소수점은 Fab 전체에서 평균화된 기대값이며 실제 wafer 한 장에서 HBM4 0.75개를 물리적으로 만든다는 뜻이 아니다.
...
Gross 765 dies/wafer, KGD 650 dies/wafer, assembly yield 90%는 모두 `MODELED_BASELINE`이다. 실제 die map, edge exclusion, wafer sort, TSV test, 적층수율이 들어오면 교체한다. 그 전까지 화면에서는 `계획 HBM4 12-Hi stack 환산`이라고 표시하며 실제 출하량으로 부르지 않는다.
...
이 문서가 M20 생산 정의의 최신 기준이다. 다만 현재 `src/lib/fab-scenario.ts`에는 이전 `2026-PLAN-V1`의 M20 `50,000 WSPM`이 남아 있다. 문서 승인과 코드·DB 마이그레이션은 별도 단계로 진행한다.

- 문서 기준: M20 NORMAL 117K / 명목 130K / 정상 WIP 16,380 FOUP
- 현재 코드: M20 명목 50K / 12개 VISUAL FOUP 중심
- 금지: 코드 숫자만 130K로 바꾸고 기존 HBM 월사용량을 일괄 2.6배 하는 것
- 다음 연결: 자재 원단위 마스터 → 시나리오 월소요량 → 재고·안전재고·창고 Capacity 순서
