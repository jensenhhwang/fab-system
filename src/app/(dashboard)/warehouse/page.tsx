...
const TYPE_DESC: Record<string, string> = {
  ...
  MRO: "공구·부품 소형 슬롯 보관",
  BULK_GAS: "물질별 벌크탱크 · 기화기 · 정제기 · Main Header",
        ...
        {wh.byCategory.map((c) => (
          <div key={c.category} className="flex items-center gap-1">
            ...
            <span className="text-[10px] text-[#999]">
              {wh.capacityMode === "SPACE" ? `${c.occupancy.toLocaleString()} ${wh.unit}` : `${c.occupancy.toFixed(1)}%p`}
              {` (${((c.occupancy / total) * 100).toFixed(1)}%)`}
            </span>
          </div>
        ))}
      </div>
    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border-t-4 border-[#0078D4]">
          <div className="text-[11px] text-[#999] mb-1 font-medium">운영 보관·공급시설</div>
          ...
          <Link key={wh.id} href={`/warehouse/${wh.code}`}
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#0078D4] text-sm opacity-40 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all">→</span>
              </div>
            </div>

            <div className="text-[10px] text-[#999] mb-1">
              {wh.capacityMode === "TANK_LEVEL" ? "평균 탱크 레벨" : wh.capacityMode === "CONTINUOUS" ? "연속공급 가동상태" : "공간 점유율"}
              {` (현재 ${wh.occupancy.toLocaleString()} / 총 ${wh.totalCapacity.toLocaleString()} ${wh.unit})`}
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-[#F0F2F5] text-[10px] font-bold text-[#0078D4] opacity-70 group-hover:opacity-100">
              상세 3D 보관 위치 보기
            </div>
          </Link>
        ))}
